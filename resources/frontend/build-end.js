...
const distDir = path.resolve(__dirname, '../../public/static/dist');
const templateJs = path.resolve(__dirname, '../views/layouts/_js.blade.php');
const templateCss = path.resolve(__dirname, '../views/layouts/_css.blade.php');
    ...
    travel(src, function(file){
        if(!endWith(file, '.map')){
            ...
            if(type === 'css'){
                ...
                cssArr.push(`<link href="{{ asset('static/`+ fileUri +`') }}" rel="stylesheet">`);
            }
            if(type === 'js'){
                fileUri = 'dist/js/' + fileName;
                hash.js[key] = fileUri;
                if(key === 'main'){
                    jsMain = `<script src="{{ asset('static/`+ fileUri +`') }}"></script>`;
                    ...
                    jsvendor = `<script src="{{ asset('static/`+ fileUri +`') }}"></script>`;
                }
            }
        }
    })
